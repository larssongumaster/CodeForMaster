...
function pilotGibbs(Y, dt, error, chains, start, particles, iterations, priors, vars, vars_d, log_scale=false);
    ...
    # individual level #
    individual = zeros(2, subjects, chains, iterations)

    # population level #
    population = zeros(2, chains, iterations)

    # common parameters #
    common = zeros(chains, iterations)

    for c in 1:chains
        individual[1,:,c,1] .= start[1,c]
        individual[2,:,c,1] .= start[2,c]
    
        population[1,c,1] = start[1,c]
        population[2,c,1] = start[2,c]
    
        common[c,1] = start[3,c]
    end
    ...
    for c in 1:chains
        ...
        t = time_ns()
        # for conjugacy
        con_prior1 = Normal(mean(priors[1]), sqrt(var(priors[1])))
        con_prior2 = Normal(mean(priors[2]), sqrt(var(priors[2])))
        ...
        likelihood[c,1] = sum([particle_filter(particles, Y[j,:], [individual[1,j,c,1], individual[2,j,c,1], common[c,1]], dt, error, log_scale) for j in 1:subjects])
        ...
        for i in iter
            ...
            # individual level #
            for j in 1:subjects
                prop_ind = [rand(Normal(individual[1,j,c,i-1], vars[1])),rand(Normal(individual[2,j,c,i-1], vars[2]))]
                prop_ind_likelihood = particle_filter(particles, Y[j,:], [prop_ind[1], prop_ind[2], common[c,i-1]], dt, error, log_scale)
                
                a = min(0, (prop_ind_likelihood         + logpdf(Normal(population[1,c,i-1], vars[1]), prop_ind[1])           + logpdf(Normal(population[2,c,i-1], vars[2]), prop_ind[2])) 
                        -  (last_accepted_likelihood[j] + logpdf(Normal(population[1,c,i-1], vars[1]), individual[1,j,c,i-1]) + logpdf(Normal(population[2,c,i-1], vars[2]), individual[2,j,c,i-1])))

                if log(rand(Uniform(0,1))) < a
                    individual[:,j,c,i] .= prop_ind
                    last_accepted_likelihood[j] = prop_ind_likelihood
                else
            ...
            # common parameters #
            prop_common = rand(Normal(common[c,i-1], vars[3]))

            prop_common_likelihood = logpdf(priors[3],prop_common)
            last_accepted_common_likelihood = logpdf(priors[3],common[c,i-1])

            for j in 1:subjects
                prop_common_likelihood = prop_common_likelihood                   + particle_filter(particles, Y[j,:], [individual[1,j,c,i], individual[2,j,c,i], prop_common],   dt, error, log_scale)
                last_accepted_common_likelihood = last_accepted_common_likelihood + particle_filter(particles, Y[j,:], [individual[1,j,c,i], individual[2,j,c,i], common[c,i-1]], dt, error, log_scale)
            end
            ...
            # population level #
            posterior1 = conjugacy(con_prior1, vars_d[1], individual[1,:,c,i]);
            population[1,c,i] = rand(posterior1)
            con_prior1 = posterior1

            posterior2 = conjugacy(con_prior2, vars_d[2], individual[2,:,c,i]);
            population[2,c,i] = rand(posterior2)
            con_prior2 = posterior2

            likelihood[c,i] = sum([particle_filter(particles, Y[j,:], [individual[1,j,c,i], individual[2,j,c,i], common[c,i]], dt, error, log_scale) for j in 1:subjects])
...
function pilotAD(Y, dt, error, chains, start, particles, iterations, log_scale = false);
    ...
    subjects = length(Y[:,1])
    result = zeros(subjects*2+3, chains, iterations)    
    likelihood = zeros(chains, iterations)
    time = zeros(chains)

    fun(p) = -1*sum([particle_filter(particles, Y[i,:], [p[i], p[subjects+i], p[subjects*2+3]], dt, error, log_scale) for i in 1:subjects])

    for i in 1:chains

        t = time_ns()

        start_long = zeros(subjects*2+3)
        for j in 1:subjects
            start_long[j] = start[1,i]
            start_long[subjects+j] = start[2,i]
        end
        start_long[subjects*2+1] = start[1,i]
        start_long[subjects*2+2] = start[2,i]
        start_long[subjects*2+3] = start[3,i]
        ...
        for j in iter
            ...
            result[subjects*2+1,i,j] = mean(result[1:subjects,i,j])
            result[subjects*2+2,i,j] = mean(result[(subjects+1):(subjects*2),i,j])
...
function pilotComp(path, true_val, Y, subjects, dt, error, chains, ratio, particles, iterations, prior, vars, vars_d);
    ...
    plot_path(pop[1,:,:],"alpha",true,true_val[1])
    savefig(path*"/MH_alpha.png")
    plot_path(pop[2,:,:],"beta",true,true_val[2])
    savefig(path*"/MH_beta.png")
    plot_path(com,"sigma",true,true_val[3])
    ...
    plot_path(exp.(rAD[subjects*2 + 1,:,:]),"alpha",false,true_val[1])
    savefig(path*"/AD_alpha.png")
    plot_path(exp.(rAD[subjects*2 + 2,:,:]),"beta",false,true_val[2])
    savefig(path*"/AD_beta.png")
    plot_path(exp.(rAD[subjects*2 + 3,:,:]),"sigma",false,true_val[3])
    ...
    # plot comparison #
    plot_comp(pop[1,:,:], exp.(rAD[subjects*2 + 1,:,:]),iterations3,"alpha",true_val[1])
    savefig(path*"/comp_alpha.png")
    plot_comp(pop[2,:,:], exp.(rAD[subjects*2 + 2,:,:]),iterations3,"beta",true_val[2])
    savefig(path*"/comp_beta.png")
    plot_comp(com, exp.(rAD[subjects*2 + 3,:,:]),iterations3,"sigma",true_val[3])
...
path = "mixed21";
r, a, l = pilotComp(path, theta, Y, subjects, dt, error, chains, ratio, particles, iterations, prior, vars, theta_v);
